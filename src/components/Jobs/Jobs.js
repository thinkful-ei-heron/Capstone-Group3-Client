...
export default class Jobs extends Component {
  ...
  handleApprovalSubmit = async (id, status, approval = false) => {
    let jobs = [];
    console.log(id);
    await this.context.updateJobStatus(id, status, this.props.job.project_id, approval);
  ...
  renderProjectButtons(approval, total_hours, hours_completed, id, status) {
    ...
    if (this.context.user.role === 'project worker') {
      if (status === 'completed' && status !== 'submitted') return <span>Project Completed</span>;
    ...
    if (this.context.user.role === 'project manager' || this.context.user.role === 'admin') {
