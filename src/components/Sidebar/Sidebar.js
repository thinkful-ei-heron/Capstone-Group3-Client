...
export default class Sidebar extends Component {
  constructor(props) {
    ...
    this.state = {
      userType: 'owner',
      employees: [{Bill: [{'project one': 'Make Bread'}, {'project two': 'Clean Oven'}]},
                  {Ben: [{'project one': 'Prepare Egg Wash'}]},
                  {Betsy: [{'project two': 'Mop floors'}]}
    ],
      projectManagers: [{Dave: ['project one', 'project three']}, {Darla: ['project two']}],
      expanded: []
    }
  }
  ...
  renderSubList = (values) => {
    if(this.state.userType === 'owner' && this.props.view !== 'project') {
      ...
      return values[0].map((project, index) => {
        return <li key={index}>
            <button onClick={e => this.toggleExpand(e)} id={Object.keys(project)[0]+index}>expand</button>
  ...
  renderList = () => {
    if(this.state.userType === 'owner' && this.props.view !== 'project') {
      return this.state.projectManagers.map((manager, index) => {
        return <li key={index}>
            <button onClick={e => this.toggleExpand(e)} id={Object.keys(manager)[0]+index}>expand</button>
            <h5>{Object.keys(manager)[0]}</h5>
            {this.state.expanded.includes(Object.keys(manager)[0]+index) ?
            <ul>
              {this.renderSubList(Object.values(manager))}
            </ul> :
            <></>
            }
          </li>;
      ...
      return this.state.employees.map((employee, index) => {
        return <li key={index}>
            <button onClick={e => this.toggleExpand(e)} id={Object.keys(employee)[0]+index}>expand</button>
            <h5>{Object.keys(employee)[0]}</h5>
            {this.state.expanded.includes(Object.keys(employee)[0]+index) ?
            <ul>
              {this.renderSubList(Object.values(employee))}
            </ul> :
            <></>
            }
          </li>
  ...
  render() {
    if(this.state.userType === 'employee' && this.props.view !== 'project') return <></>
    else {
      return(
        <div>
