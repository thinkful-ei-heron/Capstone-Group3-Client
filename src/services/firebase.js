...
class Firebase {
  ...
  setJobs = (role, name) => {
    this.state.projects.forEach(project => {
      this.db
        .collection(`organization/${this.state.user.org.id}/projects/${project.id}/jobs`)
        .get()
        .then(snapshot => {
          const jobs = [];
          if (role === 'project worker') {
  ...
  getJobs = project_id =>
    this.db.collection(`organizations/HkeHO8n1eIaJSu6mnsd5/projects/${project_id}/jobs`).get();
